...
const (
	...
	ErrorsSoftLinkCreateFail        string = "自动创建软连接失败，请以管理员身份运行客户端 (开发环境为 goland 等，生产环境检查命令执行者权限), " +
		"最后一个可能：如果您是 360 用户，请退出 360 相关软件，才能保证 go 语言创建软连接函数：os.Symlink() 正常运行"
	...
	ErrorsParseTokenFail        string = "解析 token 失败"
	ErrorsTokenBaseInfo         string = "token 最基本的格式错误，请提供一个有效的 token!"
	ErrorsNoAuthorization       string = "token 鉴权未通过，请通过 token 授权接口重新获取 token,"
	ErrorsRefreshTokenFail      string = "token 不符合刷新条件，请通过登陆接口重新获取 token!"
	...
	ErrorsWebsocketBeatHeartFail              string = "websocket BeatHeart 心跳协程出错"
	...
	ErrorsWebsocketClientOfflineTimeout       string = "websocket 客户端响应 ping 消息超过服务端允许的最长时间 (秒):"

	// rabbitMq
	ErrorsRabbitMqReconnectFail string = "RabbitMq 消费者端掉线后重连失败，超过尝试最大次数"

	// 文件上传
	ErrorsFilesUploadOpenFail string = "打开文件失败，详情："
	ErrorsFilesUploadReadFail string = "读取文件 32 字节失败，详情："

	// casbin 初始化可能的错误
	ErrorCasbinCanNotUseDbPtr         string = "casbin 的初始化基于 gorm 初始化后的数据库连接指针，程序检测到 gorm 连接指针无效，请检查数据库配置！"
